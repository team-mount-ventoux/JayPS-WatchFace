      ...
      if (s_live.nb > 0) {
        if (cell_index->row < s_live.nb) {
          if (s_live.sorted_friends[cell_index->row]->distance < 1000) {
            snprintf(subtitle, sizeof(subtitle), "%dm", (int) s_live.sorted_friends[cell_index->row]->distance);
          } else {
            snprintf(subtitle, sizeof(subtitle), "%ld.%ldkm", s_live.sorted_friends[cell_index->row]->distance / 1000, s_live.sorted_friends[cell_index->row]->distance % 1000 / 100);
          }
          snprintf(tmp, sizeof(tmp), "%s %u°", subtitle, s_live.sorted_friends[cell_index->row]->bearing);
          if (s_live.sorted_friends[cell_index->row]->lastviewed < 60) {
            snprintf(subtitle, sizeof(subtitle), "%s %d\"", tmp, s_live.sorted_friends[cell_index->row]->lastviewed);
          } else {
            snprintf(subtitle, sizeof(subtitle), "%s %d'%d\"", tmp, s_live.sorted_friends[cell_index->row]->lastviewed / 60, s_live.sorted_friends[cell_index->row]->lastviewed % 60);
          }
          snprintf(s_live.sorted_friends[cell_index->row]->subtitle, sizeof(s_live.sorted_friends[cell_index->row]->subtitle), "%s", subtitle);
...
void screen_live_layer_init(Window* window) {
  ...
  s_live.friends[0].bearing = 88;
  s_live.friends[0].lastviewed = 53;
  s_live.friends[0].xpos = 20;
  s_live.friends[0].ypos = 50;
  strcpy(s_live.friends[1].name, "Nic");
  s_live.friends[1].distance = 1800;
  s_live.friends[1].bearing = 123;
  s_live.friends[1].lastviewed = 125;
  s_live.friends[1].xpos = -80;
  s_live.friends[1].ypos = 100;
